...
const applyVendor = require('./transforms/vendorify');

module.exports = writeBundles;

function createAllBundles(mainFiles, transforms, events, options) {
  let bundles = [];
...
function createVendoredBundles(mainFiles, transforms, events, options) {
  let bundles = [];

  let vendorFiles = new FileSet();
  vendorFiles.entryFile = mainFiles.entryFile;
  vendorFiles.outputFile = util.appendToFileName(mainFiles.outputFile, '.vendor');
  if (options.debug) {
    vendorFiles.mapFile = vendorFiles.outputFile + '.map';
  }

  createAllBundles(vendorFiles, transforms, events, options).forEach(browserify => {
    browserify.plugin(applyVendor, transforms.vendorify);
    bundles.push(browserify);
  })

  // non-vendor bundles
  let nonVendorOptions = Object.assign({}, options, {bundleExternal: false});
  bundles.push.apply(bundles, createAllBundles(mainFiles, transforms, events, nonVendorOptions));
...
function writeBundles (mainFiles, events, options) {
  ...
  if(options.vendor) {
    bundles.push.apply(bundles, createVendoredBundles(mainFiles, transforms, events, options));
  }
  else {
    bundles.push.apply(bundles, createAllBundles(mainFiles, transforms, events, options));
  }
